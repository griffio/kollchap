...
if [ -n "$PIDS" ]
then
  echo "Kollchap is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/kollchap.out
ERR_FILE=${LOGGING_PATH}/kollchap.err
...
echo "Giving Kollchap $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Kollchap didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Kollchap failed"
else
  echo "Starting Kollchap succeeded"
fi
